...
task  slave_monitor :: capture_write_address();
 ...
 forever begin
    `uvm_info("slave_monitor :: capture_write_address","Triggred",UVM_LOW);
    ...
    `uvm_info("slave_monitor :: capture_write_address",pkt.sprint(),UVM_LOW);
...
task  slave_monitor ::  capture_write_data();
 ...
 write_data_mbx =    new ();
 forever begin
    `uvm_info("slave_monitor :: capture_write_data","Triggred",UVM_LOW);
    pkt = slave_seq_item :: type_id :: create("pkt");
    ...
    do begin
      ...
      //`uvm_info("slave_monitor :: capture_write_data","captured data pkt put to write_data_mbx",UVM_LOW);
...
task  slave_monitor ::  capture_write_response();
 slave_seq_item     pkt;
 forever begin
    `uvm_info("slave_monitor :: capture_write_response","Triggred",UVM_LOW);
    ...
    `uvm_info("slave_monitor :: capture_write_response","IamInside",UVM_LOW);
    ...
    if(wresp_array.exists(slave_mon_intf.slv_mon_cb.bid)) begin    //to check if captured bid matches with any transaction waiting for response.
       ...
       //`uvm_info("slave_monitor :: capture_write_response","Sending pkt to SB",UVM_LOW);
...
task  slave_monitor :: capture_read_address();
 slave_seq_item     pkt;
 forever begin
    `uvm_info("slave_monitor :: capture_read_address","Triggred",UVM_LOW);
    pkt = slave_seq_item :: type_id :: create("pkt");
    ...
    //`uvm_info("slave_monitor :: capture_read_address",$sformatf("captured address pkt put to read_address_array arid=%d",pkt.arid),UVM_LOW);
...
task  slave_monitor ::  capture_read_data();
 ...
 bit last;
 forever begin
    `uvm_info("slave_monitor :: capture_read_data","Triggred",UVM_LOW);
    i = 0;
    do begin
      pkt = slave_seq_item :: type_id :: create("pkt");
   ...
   for(i=0 ; i<no_of_beats; i++)begin         //merges all beats with same rid to pkt having address info
    ...
    //`uvm_info("slave_monitor :: capture_read_data","Sending pkt to SB",UVM_LOW);
...
int x ,no_addr, no_data; //indicates number of completed write transactions waiting for response.
  `uvm_info("slave_monitor :: merge_write_info","Triggred",UVM_LOW);
  ...
  //adding packets to array waiting for response
    //`uvm_info("slave_monitor :: merge_write_info",$sformatf("putting merged pkt to wresp_array awid = %b",merged_pkt.awid),UVM_LOW);
...
task slave_monitor :: capture_reset();
 ...
 `uvm_info("slave_monitor :: capture_reset","Triggred",UVM_LOW);
 fork
  forever begin //reset deasserted
    ...
    `uvm_info("slave_monitor :: capture_reset","reset_deasserted_pkt to SB",UVM_LOW);
  ...
  forever begin //reset asserted
    ...
    rst_pkt.reset_deasserted = $realtime();
    `uvm_info("slave_monitor :: capture_reset","reset_asserted_pkt to SB",UVM_LOW);
    mon_ap.write(rst_pkt);
  end
join
endtask
...
/*                                                                        */
/* AXI_INTERCONNECT_DESIGN/AXI_TB/slave_monitor.sv                        */
/*                                                                        */
/* RAITON CONFIDENTIAL                                                    */
/*                                                                        */
...
task  slave_monitor :: capture_write_address();
 ...
 forever begin
    ...
    wait(slave_mon_intf.slv_mon_cb.awvalid && slave_mon_intf.areset_n==1); //wait for valid
    pkt.awaddr   = slave_mon_intf.slv_mon_cb.awaddr;
    `uvm_info("slave_monitor wr_addr_pkt",$sformatf("pkt.awaddr=%0h slave_mon_intf.slv_mon_cb.awaddr=%0h",pkt.awaddr,slave_mon_intf.slv_mon_cb.awaddr),UVM_LOW);  
    pkt.awburst  = burst_type_t'(slave_mon_intf.slv_mon_cb.awburst);
    pkt.awcache  = slave_mon_intf.slv_mon_cb.awcache;
    pkt.awid     = slave_mon_intf.slv_mon_cb.awid;
    pkt.awlen    = slave_mon_intf.slv_mon_cb.awlen;
    ...
    pkt.awsize   = slave_mon_intf.slv_mon_cb.awsize;
    pkt.operation = WRITE;
    resp_ap.write(pkt); 		//write pkt to sequence via seqr for response
    wait( slave_mon_intf.slv_mon_cb.awready && slave_mon_intf.slv_mon_cb.awvalid && slave_mon_intf.areset_n==1); //wait for handshake
    pkt.wadd_hndshk = $realtime();  	//capturing timestamp for handshake.
    //`uvm_info("slave_monitor :: capture_write_address","captured address pkt put to write_address_mbx",UVM_LOW);
  //  `uvm_info("slave_monitor_wr_addr_pkt",pkt.sprint(),UVM_LOW);
    write_address_mbx.put(pkt);
    @(slave_mon_intf.slv_mon_cb); 	//wait till next clk posedge
  end
endtask
...
task  slave_monitor ::  capture_write_data();
 ...
 forever begin
    ...
    //`uvm_info("slave_monitor :: capture_write_data","captured data pkt put to write_data_mbx",UVM_LOW);
 //   `uvm_info("slave_monitor_wr_data_pkt",pkt.sprint(),UVM_LOW);
    write_data_mbx.put(pkt);
 end
endtask
...
task  slave_monitor ::  capture_write_response();
 ...
 forever begin
    ...
    if(wresp_array.exists(slave_mon_intf.slv_mon_cb.bid)) begin    //to check if captured bid matches with any transaction waiting for response.
       ...
       `uvm_info("slave_monitor :: capture_write_response","Sending pkt to SB",UVM_LOW);
       //pkt.print_write_txn(pkt);
       mon_ap.write(pkt); //wr addr+data+response write pkt to sb
    `uvm_info("slave_monitor_capture_write_info",pkt.sprint(),UVM_LOW);
    end else `uvm_error("slave_monitor :: capture_write_response",$sformatf("unexpected write response, BID not found bid= %b",slave_mon_intf.slv_mon_cb.bid))
    @(slave_mon_intf.slv_mon_cb); //wait till next clk posedge
 end
endtask
...
task  slave_monitor :: capture_read_address();
 ...
 forever begin
    ...
    pkt.arsize   = slave_mon_intf.slv_mon_cb.arsize;
    pkt.operation= READ;
    //`uvm_info("slave_monitor_capture_read_addr",pkt.sprint(),UVM_LOW);
    resp_ap.write(pkt); 		//write pkt to sequence via seqr for response
    wait( slave_mon_intf.slv_mon_cb.arready==1 && slave_mon_intf.slv_mon_cb.arvalid==1 && slave_mon_intf.areset_n==1); //handshake
    pkt.radd_hndshk = $realtime();  	//capturing timestamp for handshake.
    //`uvm_info("slave_monitor :: capture_read_address",$sformatf("captured address pkt put to read_address_array arid=%d",pkt.arid),UVM_LOW);
...
task  slave_monitor ::  capture_read_data();
 ...
 forever begin
    ...
    //`uvm_info("slave_monitor :: capture_read_data","Sending pkt to SB",UVM_LOW);
    //`uvm_info("slave_monitor_capture_read_data",pkt2sb.sprint(),UVM_LOW);
    //pkt2sb.print_read_txn(pkt2sb);
    mon_ap.write(pkt2sb); //read addr+data+resp write pkt to sb
 end
endtask
