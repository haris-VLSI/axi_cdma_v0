class slave_monitor extends uvm_monitor;
   ...
   uvm_analysis_port #(slave_seq_item)   s_mon_ap;
...
task  slave_monitor ::  capture_write_response();
 ...
 forever begin
    ...
    //if(wresp_array.exists(slave_mon_intf.slv_mon_cb.bid)) begin    //to check if captured bid matches with any transaction waiting for response.
       pkt = slave_seq_item :: type_id :: create("pkt");
       //wresp_array[slave_mon_intf.slv_mon_cb.bid].get(pkt);  
   //// `uvm_info("after writing to scorebaord",pkt.sprint(),UVM_LOW);         //this pkt already has add + data info
   ...
   /// `uvm_info("slave_monitor :: capture_write_response ",$sformatf(" after writing pkt2sb %p",pkt),UVM_LOW);
...
task  slave_monitor :: capture_read_address();
 ...
 forever begin
    ...
    pkt = slave_seq_item :: type_id :: create("pkt");
    `uvm_info("slave_monitor :: capture_read_address","Waiting for valid",UVM_LOW);
    wait(slave_mon_intf.slv_mon_cb.arvalid==1 && slave_mon_intf.areset_n==1); //wait for valid
    `uvm_info("slave_monitor :: capture_read_address","GOT Valid",UVM_LOW);
    ...
    pkt.operation = READ;
    resp_ap.write(pkt); //write pkt to sequencer for response
////    `uvm_info("after writing to scorebaord",pkt.sprint(),UVM_LOW);
...
task  slave_monitor ::  capture_read_data();
 ...
 forever begin
   ...
   for(i=0 ; i<no_of_beats; i++)begin         //merges all beats with same rid to pkt having address info
     ...
     s_mon_ap.write(pkt2sb);
    //`uvm_info("slave_monitor :: capture_read_data","Sending pkt to SB",UVM_LOW);
    //pkt2sb.print_read_txn(pkt2sb);
   // `uvm_info("slave_monitor :: capture_read_data ",$sformatf(" before writing pkt2sb %p",pkt2sb),UVM_LOW);
    mon_ap.write(pkt2sb); //write pkt to sb
   /// `uvm_info("slave_monitor :: capture_read_data ",$sformatf(" after writing pkt2sb %p",pkt2sb),UVM_LOW);
 end
endtask
