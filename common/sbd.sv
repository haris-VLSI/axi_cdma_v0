class cdma_sbd extends uvm_scoreboard;
    ...
    task predict_master_packet();
        `uvm_info("SBD","get_master_packets",UVM_LOW)
        forever begin
            ...
            if(cfg_pkt.awaddr == 'h28) begin
                ...
                transfer_active = 1;
                `uvm_info("SBD_PRE",$sformatf("cr_t configured: %0h", cr_t),UVM_LOW)
                `uvm_info("SBD_PRE",$sformatf("sr_t configured: %0h", sr_t),UVM_LOW)
                `uvm_info("SBD_PRE",$sformatf("sa_loc configured: %0h", sa_loc),UVM_LOW)
                `uvm_info("SBD_PRE",$sformatf("da_loc configured: %0h", da_loc),UVM_LOW)
                `uvm_info("SBD_PRE",$sformatf("btt_b configured decimal: %0d. Transfer Active!", btt_b),UVM_LOW)
            //ARLength Prediction
                burst_size = 2**4;
    ...
    task compare_master_pkt();
        ...
        `uvm_info("SBD","get_slave_packet",UVM_LOW)
        forever begin
            ...
            if(slv_pkt.operation == READ) begin
                ...
                if(slv_pkt.arlen == exp_len_r) begin
                    `uvm_info("SBD_RAL_CHK", $sformatf("ARLEN is matched: %0h", slv_pkt.arlen), UVM_LOW)
                end
                else begin
                    `uvm_error("SBD_RAL_CHK", $sformatf("ARLEN mismatch! Expected: %0h, Got: %0h", exp_len_r, slv_pkt.arlen)) 
                end

                //CDMACR for burst type
                if(slv_pkt.arburst == exp_burst_r) begin
                    `uvm_info("SBD_RAL_CHK", $sformatf("ARBURST is matched: %p", burst_type_t'(slv_pkt.arburst)), UVM_LOW)
                end
                else begin
                    `uvm_error("SBD_RAL_CHK", $sformatf("ARBURST mismatch! Expected: %p, Got: %p", exp_burst_r, slv_pkt.arburst)) 
                end
                //SA for ARADDR
                if(sa_loc != slv_pkt.araddr) begin
                    `uvm_error("SBD_RAL_CHK", $sformatf("ARADDR mismatch! Expected: %0h, Got: %0h", sa_loc, slv_pkt.araddr)) 
                end
                else begin
                    `uvm_info("SBD_RAL_CHK", $sformatf("ARADDR match. Expected: %0h | Got: %0h",sa_loc,slv_pkt.araddr),UVM_LOW)
                end
            end
            else if(slv_pkt.operation == WRITE) begin
                //ArLen using SA and BTT
                if(slv_pkt.awlen == exp_len_w) begin
                    `uvm_info("SBD_RAL_CHK", $sformatf("AWLEN is matched: %0h", slv_pkt.awlen), UVM_LOW)
                end
                else begin
                    `uvm_error("SBD_RAL_CHK", $sformatf("AWLEN mismatch! Expected: %0h, Got: %0h", exp_len_w, slv_pkt.awlen)) 
                end

                //CDMACR for burst type
                if(slv_pkt.awburst == exp_burst_w) begin
                    `uvm_info("SBD_RAL_CHK", $sformatf("AWBURST is matched: %p", burst_type_t'(slv_pkt.awburst)), UVM_LOW)
                end
                else begin
                    `uvm_error("SBD_RAL_CHK", $sformatf("AWBURST mismatch! Expected: %p, Got: %p", exp_burst_w, slv_pkt.awburst)) 
                end
                //DA for AWADDR
                //if(da_loc != slv_pkt.awaddr) begin
                //    `uvm_error("SBD_RAL_CHK", $sformatf("AWADDR mismatch! Expected: %0h, Got: %0h", da_loc, slv_pkt.awaddr)) 
                //end
                //else begin
                //    `uvm_info("SBD_RAL_CHK", $sformatf("AWADDR match. Expected: %0h | Got: %0h",da_loc,slv_pkt.awaddr),UVM_LOW)
                //end
            end
            // ---------------------------------------------------------
            // WRITE CHANNEL CHECKER (AW)
            // ---------------------------------------------------------
            if(slv_pkt.operation == WRITE) begin
                if(expected_awaddr != slv_pkt.awaddr) begin
                    `uvm_error("SBD_AWADDR_CHK", $sformatf("Mismatch! Exp: %0h, Got: %0h", expected_awaddr, slv_pkt.awaddr)) 
                end else begin
                    `uvm_info("SBD_AWADDR_CHK", $sformatf("Match! Address: %0h", expected_awaddr), UVM_LOW)
                end
                ...
                if (remaining_write_bytes < 0) begin
                    `uvm_error("SBD_BTT_OVERFLOW", $sformatf("Hardware wrote %0d MORE bytes than BTT configured!",remaining_write_bytes))
                end
            end
        end
    endtask
